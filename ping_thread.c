...
ping(void)
{
	...
	snprintf(request, sizeof(request) - 1,
			...
			config_get_config()->gw_id, /*hmm shall we ping for all known clients as multi-client aswell -> better let the clients ping on their own*/
			sys_uptime,
			sys_memfree,
			sys_load,
	...
	//shutdown(sockfd, 2);//stop sending or receiving on this socket!! (i.e. make sure close will work *G)	
	close(sockfd);
