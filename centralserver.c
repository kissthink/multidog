...
*/
t_authcode
auth_server_request(t_authresponse *authresponse, const char *request_type, const char *ip, const char *mac, const char *token, unsigned long long int incoming, unsigned long long int outgoing, const char * gw_id)
{
	int sockfd;
	ssize_t	numbytes;
	size_t totalbytes;
...
int _connect_auth_server(int level) {
	...
	if (!h_addr) {
		...
		debug(LOG_INFO, "ipv4 socket fd: %d",sockfd);
		...
		if (connect(sockfd, (struct sockaddr *)&their_addr, sizeof(struct sockaddr)) == -1) {
			 ...
			 */
			debug(LOG_DEBUG, "Level %d: Successfully connected to auth server %s:%d (fd: %d)", level, hostname, auth_server->authserv_http_port, sockfd);
			return sockfd;
		}
	}
}
